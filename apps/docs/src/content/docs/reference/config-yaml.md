...
| `secret_scan`          | `boolean`  | `true`                          | Scans any content crab'd is about to commit (working-tree changes or a recorded memory) for secrets before it reaches the forge, refusing the commit on a finding. Worth locking at the org level like `write`.                               |
...
When memory is on but any of that is missing, crab'd warns on its tracking comment at the _start_ of
