  ...
  crabd:
    ...
    steps:
      - uses: actions/checkout@v4
        with:
          ref: ${{ github.event.pull_request.head.sha || (github.event.issue.pull_request && format('refs/pull/{0}/head', github.event.issue.number)) || '' }}
          fetch-depth: 0
...
- **Review its work**: review that pull request. On GitHub, crab'd answers every conversation and
  pushes a commit to the branch. On Forgejo, ask for the same thing with
  `/crabd implement address the review`.
