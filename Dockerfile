...
# repos. ca-certificates: TLS for API/model calls. gh: lets the agent read a single file from
# another repo cheaply (`gh api`) instead of cloning it; installed from GitHub's official apt repo.
RUN apt-get update \
